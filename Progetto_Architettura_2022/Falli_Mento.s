...
.bss
tmp_str: .skip 128                                              //passa i dati statici per acquisire intere strinhe   
tmp_int: .skip 8                                                //e gli riserva spazi di 128 bit per le str e 8 bit per gli int
smartphone: .skip smartphone_size_aligned * max_smartphone      //matrice di 80(<- smartphone_size_aligned) colonne * 10 righe(<- max_smartphone)
...
.macro save_to item, offset, size                               //macro che serve per salvare un nuovo array di un telefono nella matrice 
    ...
    mov x2, \size                                               //dimensione di ciò che salva
...
.macro ricorsione
    ...
    ldr x21, n_smartphone                                         //carica in x21 il contenuto di n_smartphone
        ...
        rico_errore:
            ...
            ldr x3, [x22, offset_smartphone_prezzo]
            bl printf
            adr x0,fmt_menu_lines
            bl printf
        end_:
...
Aggiungi_smartphone:
    ...
    mov x0, smartphone_size_aligned
    mul x0, x19, x0
    add x20, x20, x0
    ...
    fail_Aggiungi_smartphone:                                                            //se superiamo max_smartphone il programma stamperà un messaggio di errore
        adr x0, fmt_line
        bl printf
